...
//          std::cout << "\t" << n2 << std::endl;
  ...
  for (size_t i = 0; i < rds.size(); ++i) {
    if (debug)
      std::cout << "\t\tCircle vertices for road: " << rds[i].getName() << std::endl;
    std::vector<Edge> edges = rds[i].getRoadEdges();
    for (size_t j = 0; j < edges.size(); ++j) {
      std::string n1 = edges[j].getRoadA()
          + lexical_cast<std::string>(edges[j].getPointA());
      const bool found1 = vset.find(n1) != vset.end();
...
//  }
  ...
  int ixedges = idx;
  std::cout << "Found: " << ixedges << " tintersection edges" << std::endl;
  for (size_t i = 0; i < rds.size(); ++i) {
    if (debug)
      std::cout << "\tInside edges for road: " << rds[i].getName() << std::endl;
//    std::vector<Edge> edges = rds[i].getEdges();
    std::vector<Edge> edges = rds[i].getRoadEdges();
    for (size_t j = 0; j < edges.size(); ++j) {
      std::string n1 = edges[j].getRoadA()
      ...
      if (!map.hasIntersection(n1n2) && !map.hasIntersection(n2n1)) {
        Vertex v1 = map.getNameVertex(n1);
        Vertex v2 = map.getNameVertex(n2);
        Weight w = edges[j].getWeight();
        boost::add_edge(v1, v2, w, g);
        boost::add_edge(v2, v1, w, g);
        //if (debug && j % 45 == 0)
  ...
  std::cout << "Found: " << idx - ixedges << " inside edges" << std::endl;
  if (debug)
...
//  }
  ...
  const std::string srcGraphVertexName = routeNameDeparture
      + lexical_cast<std::string>(route.getAngleDepartureClosestMatch());
  const std::string dstGraphVertexName = routeNameArrival
      + lexical_cast<std::string>(route.getAngleArrivalClosestMatch());
  //std::cout << "srcGraphVertexName: " << srcGraphVertexName << std::endl;
  //std::cout << "dstGraphVertexName: " << dstGraphVertexName << std::endl;
  ...
  for (size_t i = 0; i < rds.size(); ++i) {
    ...
    for (size_t j = 0; j < edges.size(); ++j) {
      ...
      std::string sNd = edges[j].getRoadA() + lexical_cast<std::string>(edges[j].getPointA());
      std::string dNd = edges[j].getRoadB() + lexical_cast<std::string>(edges[j].getPointB());
      dot_file << sNd << " -> " << dNd << "\t[label=\"";

      std::map<std::string, int>::iterator it;
      it = solutionNodeToStepMap.find(sNd + dNd);
      int iWeight = (int) edges[j].getWeight(); //get(weightmap, e);
